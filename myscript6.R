...
option_list <- list(
  ...
  make_option(c("--refAlleleFreq"), type = "numeric", default = NULL, 
...
importFromVCF <- function(file,
  ...
  df <- readr::read_delim(pipe(command), col_names = c("CHROM", "POS", "REF", "ALT", 
    ...
    dplyr::filter(DP.HIGH > 0 & DP.LOW > 0) %>%
    dplyr::mutate(
...
ggplot2::ggsave(filename = "distribution.png", width = 10, height = 6)
...
ggplot2::ggsave(filename = outputPlot, plot = p, width = 10, height = 6)
